// Andee_for_Nano.cpp - Andee for Nano- Arduino IDE Library
// Annikken Pte Ltd
// Author: Muhammad Hasif

//The Annikken Andee is an app to enable users to create their own dynamic dashboard on their smartphones and tablets without any Objective C, Swift or Java programming. All they need is the Annikken Andee, a compatible Arduino/board and this library. All functions will be well documented. For any questions regarding the Annikken Andee, feel free to email us at andee@annikken.com.

#define PACKET_LEN 18
#define READBUFFERMAX 80
#define READPARTBUFFERMAX 64

//These functions are to set the Service and Characteristic UUID for the board. The Annikken Andee app will only respond to the UUID used in the Cpp file. You can change the UUID if you intend to write your own app.
...
//The most important part of the packet is the AEND byte, which is located at the end of the packet. By identifying the last byte, we can sift out any other noise and parse the data that is sent to the board.
...
class AndeeClass
{
	...
	void broadcast(void);
	//This is needed to start the advertisement of the BLE on the board
		
	void poll(void);
	//
	
	void setName(const char*);
	//This is to change the name of the device
